#!/usr/bin/env python3
"""V30 — V19 features (AUC 0.836) + threshold optimized on real labeled data.

V19 features give AUC 0.836 (highest ever) but threshold optimization fails
because it's done on pseudo-labeled validation data. This version:
1. Uses V19 features (target encoding, LangAssess, RD_Search, raw answers)
2. Splits the REAL labeled training fold into train/val for threshold optimization
3. Optimizes threshold on REAL labels (not pseudo-labels)
4. Uses nested CV: outer loop for evaluation, inner loop for threshold
"""
...
def run_v30_cv(n_folds=5):
    """Run V30 cross-validation with proper nested threshold optimization."""
    print(f"\n{'='*80}")
    print(f"V30 — V19 Features (AUC 0.836) + Threshold on Real Labels")
    print(f"{'='*80}")
    ...
    for fold, (echo_train_idx, echo_test_idx) in enumerate(skf.split(np.zeros(len(echo_indices)), echo_y)):
        ...
        # Prepare inner val features (for threshold optimization on REAL labels)
        ...
        # Calibrate on pseudo-labeled validation
        models_val = {
        ...
        # Optimize threshold on INNER VALIDATION (real labels!)
        print("  Optimizing threshold on real-labeled inner validation...")
        best_bacc_val = 0
        ...
        print(f"  Best (inner val): {best_method}, thresh={best_thresh:.3f}, pro_adj={best_pro_adj:+.3f}, val_bacc={best_bacc_val:.3f}")
...
def main():
    print("=" * 80)
    print("V30 — V19 Features (AUC 0.836) + Threshold on Real Labels")
    print("=" * 80)

    results = run_v30_cv(n_folds=5)
    ...
    print(f"  V29 (V19 feat + val opt): BAcc=0.645, AUC=0.836")
    print(f"  V30 (V19 feat + real opt): BAcc={results['avg_bacc']:.3f}, F1={results['avg_f1']:.3f}, AUC={results['avg_auc']:.3f}")
    print(f"  Gap to 90%:              {0.90 - max(results['avg_bacc'], 0.744):.3f}")

    with open(AUTOSURVEY_DIR / "v30_cv_results.json", "w") as f:
