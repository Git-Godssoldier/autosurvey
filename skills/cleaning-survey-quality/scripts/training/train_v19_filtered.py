#!/usr/bin/env python3
"""
Experiment 19: Per-dataset best selection with validation F1 + accuracy filter.

The problem with v14/v16/v17 is that validation F1 doesn't always predict
test F1 well, especially for small datasets. This experiment adds a filter:
only select an approach if its validation F1 is significantly better than
the baseline (structured-only) approach. Otherwise, fall back to the baseline.

Also: use a weighted average of validation F1 and validation accuracy as the
selection criterion, to avoid selecting approaches that sacrifice too much
accuracy for F1.
"""
...
def train_and_predict(train_df, val_df, test_df):
    ...
    # First: get baseline (structured only) validation F1
    ...
    for name, X_tr, X_va, X_te in feature_sets:
        for cfg in configs:
            ...
            # Selection criterion: F1 + 0.3 * accuracy improvement over baseline
    ...
    # Pick best by combined score, but only if F1 is better than baseline
    best_by_f1 = max(approaches, key=lambda a: a[4])
    best_by_score = max(approaches, key=lambda a: a[5])
    
    # Use F1 as primary criterion
    best = best_by_f1
